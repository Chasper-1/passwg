...
pub fn parse_args(args: &[String]) -> Config {
    ...
    while i < args.len() {
        match args[i].as_str() {
            ...
            "-w" | "--words" => {
                word_mode = true;
                if length == 16 { length = 4; } // Дефолт для фраз — 4 слова
            },
            ...
            _ => if let Ok(n) = args[i].parse::<u64>() { 
                nums.push(n); 
            } else {
    ...
    if fast_mode && word_mode {
        eprintln!("Предупреждение: флаг -f (быстрый режим) игнорируется при использовании -w (слова)");
    }
    ...
    if word_mode && length > 20 {
        eprintln!("Предупреждение: количество слов слишком большое ({})", length);
...
pub fn print_help(l: &I18n, app_name: &str, version: &str) {
    ...
    println!("{}", l.help_json);
    println!("{}", l.help_csv);
    println!("  -w, --words    {}", if l.help_usage.contains("Использование") {
